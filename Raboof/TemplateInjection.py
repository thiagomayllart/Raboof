...
def multi_post_call(path, headers, payloadslist, boundaries, original_request):
    ...
    while i < len(boundaries):
        ...
        while j < len(payloadslist[0]):
            ...
            new_thread = Thread_types.Threadti(path, headers, pay_data1, original_request, lines[0], 'POST', None, 0, list_regions)
        ...
        while j < len(payloadslist[1]):
            ...
            HTTPRequester.check_max_threads() #check if all the threads are being used
            new_thread = Thread_types.Threadti(path, headers, pay_data2, original_request, lines[0], 'POST', None, 1, list_regions)
            HTTPRequester.thread_starter(new_thread)
            lines[0] = oldline
            boundaries = oldboundaries
            j = j + 1
...
def common_post_call(path, headers, payloadslist, post_params, original_request):
    ...
    while i < len(post_params):
        ...
        while j < len(payloadslist[0]):
            ...
            new_thread = Thread_types.Threadti(path, headers, pay_data1, original_request, '='.join(vars), 'POST', None, 0, list_regions)
            HTTPRequester.thread_starter(new_thread)
        ...
        while j < len(payloadslist[1]):
            ...
            HTTPRequester.check_max_threads()
            new_thread = Thread_types.Threadti(path, headers, pay_data2, original_request, '='.join(vars), 'POST', None, 1, list_regions)
            HTTPRequester.thread_starter(new_thread)
            post_params = oldpost_params[:]
            j = j + 1
        i = i + 1
...
def get_call(path, headers, data, payloadslist, original_request):
    ...
    while i < len(params):
        ...
        while j < len(payloadslist[0]):
            ...
            HTTPRequester.check_max_threads()
            new_thread = Thread_types.Threadti(path, headers, pay_data1, original_request, var, 'GET', pay_path1,
                                                0, list_regions)
            HTTPRequester.thread_starter(new_thread)
            params = oldparams[:]
            j = j + 1
        ...
        while j < len(payloadslist[1]):
            ...
            HTTPRequester.check_max_threads()
            new_thread = Thread_types.Threadti(path, headers, pay_data2, original_request, var, 'GET', pay_path2,
                                                0, list_regions)
            HTTPRequester.thread_starter(new_thread)
            params = oldparams[:]
            j = j + 1
